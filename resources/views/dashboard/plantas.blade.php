                    ...
                    <div class="col-sm-10 col-md-10 col-lg-10 col-xl-10">
                        <select name="idnombre" id="idnombre" class="form-control @error('idnombre') is-invalid @enderror" required autocomplete="name" autofocus>
                            ...
                            @foreach($tiposPlantas as $key => $tps)
                                <option value="{{ $tps->idnombre }}" @if(old('idnombre') == "{{ $tps->idnombre }}" OR (Route::currentRouteName() == 'plantaPediaShow' AND "{{ $tps->idnombre}}" == "{{$plantaPedia->idnombre }}")) selected @endif>{{ $tps->nombre }}</option>
                            @endforeach
                    </select>
                    @error('idnombre')
